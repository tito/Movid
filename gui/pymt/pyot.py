...
class otModuleProxy(object):
    ...
    def _get_inputs(self):
        if self.daemon.describe(self.type).has_key('inputs'):
            print self.daemon.describe(self.type)['inputs']
    ...
    def _get_outputs(self):
        if self.daemon.describe(self.type).has_key('outputs'):
            print self.daemon.describe(self.type)['outputs']
...
class MovidDaemon(object):
    ...
    def connect_module_by_index(self, out_module, out_port, in_module, in_port):
        url = '/pipeline/connect?out=%s&in=%s&outidx=%s&inidx=%s' % (out_module, in_module, str(out_port), str(in_port))
